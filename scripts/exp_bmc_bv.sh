...
echo "spectre v1, $((end-start))" >> stats_ss
...
echo "spectre v2, $((end-start))" >> stats_ss
if [[ "$res" != *"1 assertions failed"* ]]; then
...
echo "spectre v4, $((end-start))" >> stats_ss
if [[ "$res" != *"1 assertions failed"* ]]; then
...
echo "meltdown, $((end-start))" >> stats_ss
if [[ "$res" != *"1 assertions failed"* ]]; then
...
start=`gdate +%s%3N`
res=`./scripts/run_poc_example_bmc_bv_reg_perm.sh fp_cache lazy_fp`
end=`gdate +%s%3N`
echo "lazy-fp, $((end-start))" >> stats_ss
if [[ "$res" != *"1 assertions failed"* ]]; then
    echo "poc bmc meltdown failed"
    exit 1
fi
...
echo "foreshadow, $((end-start))" >> stats_ss
if [[ "$res" != *"1 assertions failed"* ]]; then
...
start=`gdate +%s%3N`
res=`./scripts/run_poc_example_bmc_bv.sh cache lvi`
end=`gdate +%s%3N`
echo "lvi, $((end-start))" >> stats_ss
if [[ "$res" != *"1 assertions failed"* ]]; then
...
start=`gdate +%s%3N`
res=`./scripts/run_poc_example_bmc_bv_pc.sh power_channel netspectre`
end=`gdate +%s%3N`
...
start=`gdate +%s%3N`
res=`./scripts/run_poc_example_bmc_bv_lfb.sh cache mds`
end=`gdate +%s%3N`
echo "mds, $((end-start))" >> stats_taint
if [[ "$res" != *"1 assertions failed"* ]]; then
